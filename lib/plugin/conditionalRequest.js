/** @see https://github.com/restify/node-restify/blob/c5a07c5822c1ef52a840ae6d4ed11f2334f81472/lib/plugins/conditionalRequest.js */

'use strict';

const imLog = require('debug')('restify-wetag:if-match');
const inmLog = require('debug')('restify-wetag:if-none-match');
const imsLog = require('debug')('restify-wetag:if-modified-since');
const iumsLog = require('debug')('restify-wetag:if-unmodified-since');
...
function checkIfMatch(req, res, next) {
    ...
    if ((ifMatch = req.headers['if-match'])) {
        ...
        for (var i = 0; i < clientETags.length; i++) {
            cur = clientETags[i];
            imLog('%o', JSON.stringify({ client: cur, server: etag }));
            if (cur === '*' || cur === etag) {
                matched = true;
                break;
            }
...
function checkIfNoneMatch(req, res, next) {
    ...
    if ((ifNoneMatch = req.headers['if-none-match'])) {
        ...
        for (var i = 0; i < clientETags.length; i++) {
            cur = clientETags[i];
            inmLog('%o', JSON.stringify({ client: cur, server: etag }));
            if (cur === '*' || cur === etag) {
                matched = true;
                break;
            }
...
function checkIfModified(req, res, next) {
    ...
    imsLog('%o', JSON.stringify({ client: ctime, server: mtime }));
...
function checkIfUnmodified(req, res, next) {
    ...
    iumsLog('%o', JSON.stringify({ client: ctime, server: mtime }));
